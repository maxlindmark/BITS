...
new <- datcod |>
  ...
  mutate(version = "new",
         ...
         country = ifelse(country == "LT", "LAT", country),
         country = ifelse(country == "LV", "LTU", country))
...
dd |>
  # If I remove Sweden from the new source the match is very good!
  filter(!country == "SE") |> 
  summarise(avg = mean(avg),
            avg_st = mean(avg_st), .by = c(sub_div, year, quarter, version)) |>
...
dd |>
  # If I don't remove Sweden, there's a match between historical and new, but not Orio. This is because "SE" is not in Orio, because I make the country column from the haul ID, and the haul ID from the Swedish data does not have country. 
  summarise(avg = mean(avg),
            avg_st = mean(avg_st), .by = c(sub_div, year, quarter, version)) |>
  ggplot(aes(year, avg, color = version, linetype = version, shape = version)) +
  geom_point(size = 2, alpha = 0.5) +
...
# Compare non swedish data with Orio and the standardized density data
dd |>
  filter(!country == "SE") |> 
