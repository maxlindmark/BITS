...
pkgs <- c(
  "tidyverse", "tidylog", "RCurl", "devtools",
...
bits_ca_cod2 <- bits_ca_cod %>% map_df(., rep, .$CANoAtLngt)
...
hlcodL |> filter(is.na(length_cm)) |> distinct(CPUEun)
...
hlcodL <- hlcodL |>
  mutate(
    ...
    GroundSpeed = ifelse(is.na(GroundSpeed), mean(GroundSpeed, na.rm = TRUE), GroundSpeed),
    SweepLngt = ifelse(is.na(SweepLngt), mean(SweepLngt, na.rm = TRUE), GroundSpeed)
  )
...
# Cod
datcod <- hlcodL |>
  dplyr::select(density, biomass_density, Year, Species, ShootLat, ShootLong, Quarter, 
...
historical <- read_csv(paste0(home, "/output/cod_cpue_length_historical.csv")) |> 
  filter(year >= 1993) |> 
  filter(source == "datras") |> 
  filter(sub_div %in% c(24, 25, 26, 27, 28)) |>
  summarise(sum = sum(biomass_density), .by = c(haul_id, sub_div, year, quarter)) |> 
  summarise(mean = median(sum), .by = c(sub_div, year, quarter)) |> 
...
dd <- bind_rows(historical, new, condition)
...
dd |>
  ...
  geom_point() +
  geom_line() + 
  ggh4x::facet_grid2(quarter~sub_div, independent = "y", scales = "free")

# The differences between the series can largely be explain by 1) using different standardisation factors and 2) length-weight parameters, because I see smaller differences when I use the raw, unstandardised count cpue (not included currently in the data files)
```
