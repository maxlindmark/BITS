...
##### select just OTB
# ML note: this way of logical filtering also includes NA!
codT <- codT[codT$REDSKENG == "OTB Otter trawl bottom", ] |> drop_na(REDSKENG)
flounderT <- flounderT[flounderT$REDSKENG == "OTB Otter trawl bottom", ] |> drop_na(REDSKENG)
plaiceT <- plaiceT[plaiceT$REDSKENG == "OTB Otter trawl bottom", ] |> drop_na(REDSKENG)
dabT <- dabT[dabT$REDSKENG == "OTB Otter trawl bottom", ] |> drop_na(REDSKENG)
turbotT <- turbotT[turbotT$REDSKENG == "OTB Otter trawl bottom", ] |> drop_na(REDSKENG)
whitingT <- whitingT[whitingT$REDSKENG == "OTB Otter trawl bottom", ] |> drop_na(REDSKENG)

index <- index[index$REDSKENG == "OTB Otter trawl bottom", ] |> drop_na(REDSKENG)
...
dabT <- dabT[!dabT$Validity..haul. %in% type, ]
...
dabhistStuart <- dabhist[, c("Fartyg", "datum", "Œr", "tråltid", "område", "speed", "idx", "LATDD", "LONGDD", "Depth", "art", "lŠngd", "CPUEantal", "RSA", "RS")]
...
DataStuart_ids <- unique(DataStuart$IDX)

DataStuart <- DataStuart |>
  separate(IDX, sep = ":", into = c("IDX_SSB", "date2"), remove = FALSE) |>
  dplyr::select(-date2)

# unique(is.na(DataStuart$IDX))
# 
# t <- DataStuart |> filter(is.na(IDX))
# 
# unique(is.na(t))
# 
# t2 <- t |> 
...
floswe$estWyear <- floswe$aL * floswe$Lengthcl^floswe$b / 1000
...
# A strange thing with dab and turbot in the swedish data is that I have CPUE == 0 but !(is.na(length)). I'll delete this
# dabswe |> 
#   filter(CPUEun == 0 & !is.na(Lengthcl))

dabswe <- dabswe |> 
  filter(!(CPUEun == 0 & !is.na(Lengthcl))) |> 
  filter(!(is.na(Lengthcl) & CPUEun > 0))

# dabswe |> 
#   filter(CPUEun == 0) |> distinct(Lengthcl)
# dabswe |> 
#   filter(is.na(Lengthcl)) |> distinct(CPUEun)
...
# p1 <- FLO_LAT |> 
#   mutate(country = "Lat",
#          trawl_time = as.character(trawl_time)) |> 
#   bind_rows(floswe |> mutate(country = "Swe")) |> 
#   filter(year > 1977 & year < 2000) |> 
#   summarise(tot_catch = sum(CPUEstBIOyear), .by = c(IDX, year, country)) |> 
#   mutate(catch = ifelse(tot_catch == 0, "N", "Y")) |> 
#   summarise(prop_empty = median(catch == "N"), .by = c(year, country)) |> 
#   ggplot(aes(year, prop_empty, color = country)) + 
#   geom_line() +
#   geom_point()
# 
# # Average weight
...
#   bind_rows(floswe |> mutate(country = "Swe")) |> 
#   filter(year > 1977 & year < 2000) |> 
#   summarise(mean_cpue = median(CPUEstBIOyear), .by = c(year, country)) |> 
#   ggplot(aes(year, mean_cpue, color = country)) + 
#   geom_line() +
#   geom_point()
# 
# # Average abundance
...
#   bind_rows(floswe |> mutate(country = "Swe")) |> 
#   filter(year > 1977 & year < 2000) |> 
#   summarise(mean_cpue = median(CPUEst), .by = c(year, country)) |> 
#   ggplot(aes(year, mean_cpue, color = country)) + 
#   geom_line() +
#   geom_point()
# 
...
COD2 <- COD2 |>
  ...
  dplyr::select(year, SD, IDX, Lengthcl, species, Rect, date, LATDD, LONGDD, density, biomass_density, source, month, quarter, aL, b,
