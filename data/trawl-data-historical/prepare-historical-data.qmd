---
title: "Collate historical data 1978--1993"
author: "Max Lindmark"
date: today
date-format: iso
toc: true
...
This script reads code and data collated by Orio et al around 2017 for his PhD thesis on CPUE from various national surveys prior to DATRAS and the later gear homogenization (around 2010). I have here copied this code and put it in chunks more or less as is! I have trimmed it in places and added species. I have removed the pelagic species because I won't use the catch information, and there is no biological information ("CA") that is not on DATRAS (historical Swedish and Latvian data is focused on cod and flounder). The script saves CA data for cod and flounder in the `output` folder for cod and flounder but not the other species because if I need those I'll take them directly from DATRAS. I have added dab and turbot to Ale´s script, although I'm not sure we can say anything meaningful about the distribution of these species in the Baltic because it's pretty limited as is with catches in the Eastern Baltic Sea. If one is interested to add more species that could be relevant for the western baltic sea. But note we do not have havey any CA data that is not already on DATRAS, and catches are likely pretty low of those anyway. 

I have also kept the original folder structure. The original script is "standardization_database.R", which is available on the SLU server together with data (and in this repo). The code could be adapted to include more species. Currently it's set up more a few species but ultimate we work up only cod and flounder (which was the focus of Ale's work). We have communicated with the Latvians (April 15th 2024) and have approval to re-use the data (even if we strictly do not need the approval...).
...
stand <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Raw.csv"),sep=";",fileEncoding="windows-1252")
stand_un <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/stand_un.csv"),sep=";",fileEncoding="windows-1252")
...
stand <- stand |>
  mutate(DAB = ifelse((is.na(lŠngd) | is.na(antal)) & art == "SandskŠdda", "no", ""),
...
index <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/Research Östersjön 2.csv"),sep=";",fileEncoding="windows-1252")
cod <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_zincl_l_COD.csv"),sep=";",fileEncoding="windows-1252")
flounder <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_zincl_l_FLOUNDER.csv"),sep=";",fileEncoding="windows-1252")
plaice <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_zincl_l_PLAICE.csv"),sep=";",fileEncoding="windows-1252")

#### load data for 2015-16
index1 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/Research Östersjön 3.csv"),sep=";",fileEncoding="windows-1252")
cod1 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawl_surveys_zincl_(l)BITSCOD.csv"),sep=";",fileEncoding="windows-1252")
flounder1 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawl_surveys_zincl_(l)BITSFLOUNDER.csv"),sep=";",fileEncoding="windows-1252")
plaice1 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawl_surveys_zincl_(l)BITSPLAICE.csv"),sep=";",fileEncoding="windows-1252")
...
dab <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_zincl_l_DAB.csv"),sep=";",fileEncoding="windows-1252") |> 
  rename(Appr. = Appr..status,
...
turbot <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_zincl_l_TURBOT.csv"),sep=";",fileEncoding="windows-1252") |> 
  rename(Appr. = Appr..status,
         Validity..haul. = Validity,
         Seq..no. = Seqno,
         Subsample = Subsam,
...
whiting <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_zincl_l_WHITING.csv"),sep=";",fileEncoding="windows-1252") |> 
    rename(Appr. = Appr..status,
           Validity..haul. = Validity,
           Seq..no. = Seqno,
           Subsample = Subsam,
...
##### RECT
xx <- shapefile(paste0(here::here(), "/ICES_StatRec_mapto_ICES_Areas/StatRec_map_Areas_Full_20170124.shp"))
...
Now we read in the DATRAS data
...
datras<- readExchange(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Datras/Exchange Data_2017-03-28 11_54_40.zip"))
...
cacodfd2 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_s_Torsk.csv"),sep=";")
caflofd2 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawlsurv_s_Skrubba.csv"),sep=";")

cacodfd21 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawl_surveys_(s)BITSCOD.csv"),sep=";")
caflofd21 <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Fiskdata2/trawl_surveys_(s)BITSFLOUNDER.csv"),sep=";")
...
lwcod <- cacod |>
  ...
  left_join(HHdata |>
              dplyr::select(haul.id, lon, lat), by = "haul.id") |> # ML: adding lat-lon!
  dplyr::select(-haul.id)
...
lwcod2 <- cacodfd2 |>
  dplyr::select(Year, SD,Lengthcl,Weight, LONGDD, LATDD) |> # ML: keeping lat-lon!
  rename(lat = LATDD, 
         lon = LONGDD)
...
HH_LAT <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Datras/Historic Latvian/HH_LAT.csv"), sep=";")
...
HL_LAT <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Datras/Historic Latvian/HL_LAT.csv"), sep=";")
...
CA_LAT <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Datras/Historic Latvian/CA_LAT.csv"), sep=";")
...
CA_LATcod <- CA_LAT |>
  filter(Species=="Gadus morhua" & IndWgt != 0 & !(is.na(IndWgt))) |>
  left_join(HH_LAT |> dplyr::select(haul.id, lat, lon)) # ML: add in coordinates!
...
lwcod3 <- CA_LATcod |>
  filter(Year<=2016) |>
  dplyr::select(Year, SD,LngtClas,IndWgt, lon, lat)
...
lw_data <- bind_rows(lwcodTotal |> mutate(species = "cod"),
                     lwfloTotal |> mutate(species = "flounder"))

write_csv(lw_data, paste0(home, "/trawl-data-1978-2015/historical-trawl/output/lw_data.csv"))
```
...
lwCOD <- lwCOD |>
  ...
  mutate(b = replace_na(b, mean(b, na.rm = TRUE)),
...
newsweep <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Datras/sweep_9116.csv"),sep=";",fileEncoding="windows-1252")

sweep_LAT <- read.csv(paste0(home, "/trawl-data-1978-2015/historical-trawl/New/Datras/Historic Latvian/sweepLatvia.csv"), sep=";",fileEncoding="windows-1252")
...
names(COD)
names(COD) <- c("year", "vessel", "trawl_time", "SD", "IDX", "Lengthcl", "CPUEun", "species", 
...
COD_LAT <- HL_LATcodL[,c("Year", "Ship", "HaulDur", "SD", "haul.id", "LngtClass", "CPUEun", 
...
names(COD_LAT)
names(COD_LAT) <- c("year", "vessel", "trawl_time", "SD", "IDX", "Lengthcl", "CPUEun",
...
FLO <- hlfloL[, c("Year", "Ship", "HaulDur", "SD", "haul.id", "LngtCm", "CPUEun", "Species",
...
names(FLO)
names(FLO) <- c("year", "vessel", "trawl_time", "SD", "IDX", "Lengthcl", "CPUEun", "species", 
                "Rect", "validity", "aL", "b", "estWyear", "CPUEunBIOyear", "date", "speed", 
                "LATDD", "LONGDD", "Depth", "RSA", "RS", "CPUEst", "CPUEstBIOyear")
...
FLO_LAT <- HL_LATfloL[,c("Year", "Ship", "HaulDur", "SD", "haul.id", "LngtClass","CPUEun",
                         "Species", "Rect", "HaulVal", "aL", "b", "estWyear", "CPUEunBIOyear" )]
...
names(FLO_LAT)
names(FLO_LAT) <- c("year", "vessel", "trawl_time", "SD", "IDX", "Lengthcl", "CPUEun",
...
PLA <- hlplaL[,c("Year","Ship","HaulDur","SD", "haul.id", "LngtCm", "CPUEun", "Species", "Rect","HaulVal",
                 #"auniL" , "buni" , "aL" ,
                 "b" , #"estWuni" ,
                 "estWyear",#"CPUEunBIOuni"  ,
                 "CPUEunBIOyear" )]
...
names(PLA)
names(PLA) <- c("year" ,"vessel" ,"trawl_time", "SD", "IDX","Lengthcl" ,"CPUEun","species",  "Rect" , "validity",
                #"auniL" , "buni" , "aL"  ,
                "b" ,
                #"estWuni" ,
                "estWyear" ,
...
DAB <- hldabL[,c("Year","Ship","HaulDur","SD", "haul.id", "LngtCm", "CPUEun", "Species", "Rect","HaulVal",
                 #"auniL" , "buni" , "aL" ,
                 "b" , #"estWuni" ,
                 "estWyear",#"CPUEunBIOuni"  ,
                 "CPUEunBIOyear" )]
...
names(DAB)
names(DAB) <- c("year" ,"vessel" ,"trawl_time", "SD", "IDX","Lengthcl" ,"CPUEun","species",  "Rect" , "validity",
                #"auniL" , "buni" , "aL"  ,
                "b" ,
                #"estWuni" ,
                "estWyear" ,
...
TUR <- hlturL[,c("Year","Ship","HaulDur","SD", "haul.id", "LngtCm", "CPUEun", "Species", "Rect","HaulVal",
                 #"auniL" , "buni" , "aL" ,
                 "b" , #"estWuni" ,
                 "estWyear",#"CPUEunBIOuni"  ,
                 "CPUEunBIOyear" )]
...
names(TUR)
names(TUR) <- c("year" ,"vessel" ,"trawl_time", "SD", "IDX","Lengthcl" ,"CPUEun","species",  "Rect" , "validity",
                #"auniL" , "buni" , "aL"  ,
                "b" ,
                #"estWuni" ,
                "estWyear" ,
...
WHI <- hlwhiL[,c("Year","Ship","HaulDur","SD", "haul.id", "LngtCm", "CPUEun", "Species", "Rect","HaulVal",
                 #"auniL" , "buni" , "aL" ,
                 "b" , #"estWuni" ,
                 "estWyear",#"CPUEunBIOuni"  ,
                 "CPUEunBIOyear" )]
...
names(WHI)
names(WHI) <- c("year" ,"vessel" ,"trawl_time", "SD", "IDX","Lengthcl" ,"CPUEun","species",  "Rect" , "validity",
                #"auniL" , "buni" , "aL"  ,
                "b" ,
                #"estWuni" ,
                "estWyear" ,
...
```{r trim and save data}
COD2 |>
  filter(SD %in% c(24, 25, 26, 27, 28)) |>
  filter(year >= 1979) |>
  summarise(cpue = mean(CPUEunBIOyear), .by = c(SD, year)) |>
  ggplot(aes(year, cpue, color = SD)) +
    geom_line()
...
# Save data!
write_csv(COD2, paste0(home, "/trawl-data-1978-2015/historical-trawl/output/cod_cpue_length_historical.csv"))
write_csv(FLO2, paste0(home, "/trawl-data-1978-2015/historical-trawl/output/flounder_cpue_length_historical.csv"))
write_csv(PLA, paste0(home, "/trawl-data-1978-2015/historical-trawl/output/plaice_cpue_length_historical.csv"))
write_csv(DAB, paste0(home, "/trawl-data-1978-2015/historical-trawl/output/dab_cpue_length_historical.csv"))
write_csv(TUR, paste0(home, "/trawl-data-1978-2015/historical-trawl/output/turbot_cpue_length_historical.csv"))
```
...
COD3_sum <- COD2 |>
  tidylog::summarise(biomass_density = sum(CPUEstBIOyear / 0.45), .by = IDX) |>
  tidylog::left_join(COD2 |>
                       dplyr::select(year, SD, IDX, Lengthcl, species, Rect,
                                     date, LATDD, LONGDD, source) |>
                       distinct(IDX, .keep_all = TRUE),
            by = "IDX") |>
